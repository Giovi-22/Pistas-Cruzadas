...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 selection:bg-purple-500/30">
      ...
      <div className={`w-full max-w-4xl ${activeRoomId ? 'max-w-md mx-auto' : 'grid md:grid-cols-2'} gap-6 relative`}>
        {!activeRoomId && (
        ...
        {/* Create or Screen Card - HIDE if active room exists for players */}
        {!activeRoomId && (
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl p-8 rounded-2xl border border-slate-800 shadow-2xl hover:border-purple-500/50 transition-colors duration-300">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <h2 className="text-2xl font-bold text-slate-100">Ingresar al Juego</h2>
          </div>
          <p className="text-slate-400 mb-8 h-12">
            {activeRoomId ? `Hay una partida activa en la sala ${activeRoomId}.` : 'Ingresá el código para unirte a tu equipo.'}
          </p>
          <form onSubmit={handleJoinMobile} className="flex flex-col gap-4 relative">
            ...
            >
              <span className="relative z-10">
                {activeRoomId && roomId === activeRoomId ? 'Unirse Ahora' : 'Conectar al Juego'}
              </span>
      ...
      {/* Admin Link - ONLY show if NO room is active to avoid cluttering player view */}
      {!activeRoomId && (
        <div className="mt-12 flex flex-col items-center space-y-4">
          <p className="text-slate-600 text-xs">
            Admin tool for Cross Clues
          </p>
        </div>
      )}
    </div>
