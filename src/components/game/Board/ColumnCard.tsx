...
export const ColumnCard = ({ c, colWord }: { c: number, colWord: string }) => {
    return (
        <div key={`col-${c}`} className="flex flex-col rounded-lg overflow-hidden border-4 border-[#C9797D] shadow-xl bg-white/95 h-full w-full">
            ...
            <div className="bg-[#6EAAAD] h-[35%] flex flex-col items-center justify-center border-t-4 border-slate-800/20 px-1 py-1 text-center">
                <span className="text-white font-bold uppercase text-[1.7vh] leading-tight text-center tracking-wider drop-shadow-md">
                    {colWord}
                </span>
            </div>
        </div>
